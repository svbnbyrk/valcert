...
func ldapURL(url string) bool {
	...
	if err != nil {
		fmt.Sprintf("error parsing url %s: %v", url, err)
		return false
	}
	if u.Scheme == "ldap" {
		return true
...
func revCheck(cert *x509.Certificate) (revoked, ok bool, err error) {
	for _, url := range cert.CRLDistributionPoints {
		if ldapURL(url) {
			fmt.Sprintf("skipping LDAP CRL: %s", url)
			continue
		}

		if revoked, ok, err := certIsRevokedCRL(cert, url); !ok {
			fmt.Sprintf("error checking revocation via CRL")
			if HardFail {
				return true, false, err
			}
			return false, false, err
		} else if revoked {
			fmt.Sprintf("certificate is revoked via CRL")
			return true, true, err
		}
	}

	if revoked, ok, err := certIsRevokedOCSP(cert, HardFail); !ok {
		fmt.Sprintf("error checking revocation via OCSP")
		if HardFail {
			return true, false, err
		}
		return false, false, err
	} else if revoked {
		fmt.Sprintf("certificate is revoked via OCSP")
		return true, true, err
	}
...
func certIsRevokedCRL(cert *x509.Certificate, url string) (revoked, ok bool, err error) {
	...
	if shouldFetchCRL {
		...
		if err != nil {
			fmt.Sprintf("failed to fetch CRL: %v", err)
			return false, false, err
		}
		...
		if issuer != nil {
			...
			if err != nil {
				fmt.Sprintf("failed to verify CRL: %v", err)
				return false, false, err
			}
		}
	...
	for _, revoked := range crl.TBSCertList.RevokedCertificates {
		if cert.SerialNumber.Cmp(revoked.SerialNumber) == 0 {
			fmt.Sprintf("Serial number match: intermediate is revoked.")
			return true, true, err
		}
	}
